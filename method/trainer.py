...
class Trainer(object):
    ...
    def _save_ckpt(self, ckpt_num, update_iter):
        ...
        ob_path = os.path.join(self._config.log_dir, "observation_%08d.pkl" % ckpt_num)
        with open(ob_path, "wb") as f:
            observations = {"observation": self.all_ob_spaces}
    ...
    def train(self):
        ...
        while step < config.max_global_step:
            ...
            if runner:
                ...
                self._agent.store_episode(rollout)
                step_per_batch = mpi_sum(len(rollout["ac"]))
                self.all_ob_spaces.extend(rollout["ob"])
            else:
