...
class ReplayBufferPerStep(object):
    ...
    def sample(self, batch_size1):
        idxs = np.random.randint(
            0, self._capacity if self._full else self._idx, size=batch_size1
        )
...
class ReplayBuffer(object):
    ...
    def sample(self, batch_size1):
        # sample transitions
        transitions = self._sample_func(self._buffer, batch_size1)
        return transitions
...
class RandomSampler(object):
    ...
    def sample_func(self, episode_batch, batch_size1_in_transitions):
        rollout_batch_size1 = len(episode_batch["ac"])
        batch_size1 = batch_size1_in_transitions

        episode_idxs = np.random.randint(0, rollout_batch_size1, batch_size1)
        t_samples = [
...
class HERSampler(object):
    ...
    def sample_her_transitions(self, episode_batch, batch_size1_in_transitions):
        rollout_batch_size1 = len(episode_batch["ac"])
        batch_size1 = batch_size1_in_transitions
        ...
        transitions["r"] = np.zeros((batch_size1,))
